...
output <- file.path(userdir, "AEJApplied_20150548_replication/rep_output")
# Load the dataset
...
colnames(boot_coefs_1) <- names(coef(sureg_model_run1))
...
colnames(boot_coefs_2) <- names(coef(sureg_model_run2))
...
column_3 <- data.frame(
  ...
  `2008` = formatC(c(rbind(est_mt8_run3, se_mt8_run3)), format="f", digits = 3 ),  # Mt8 estimates + SEs
  `2005` = formatC(c(rbind(est_mt5_run3, se_mt5_run3)), format="f", digits = 3)   # Mt5 estimates + SEs
)
...
column_4 <- data.frame(
  ...
  `2008` = formatC(c(rbind(est_mt8_run4, se_mt8_run4)), format = "f", digits = 3),  # Format to 3dp
  `2005` = formatC(c(rbind(est_mt5_run4, se_mt5_run4)), format = "f", digits = 3)   # Format to 3dp
)
...
final_merged$column_priority <- match(final_merged$var_name, column_order)
